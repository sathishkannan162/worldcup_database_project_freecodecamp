...
echo "$($PSQL "SELECT SUM(opponent_goals)+SUM(winner_goals) AS total_no_of_goals FROM games")"
...
echo "$($PSQL "SELECT  DISTINCT(name) FROM teams FULL JOIN games A ON teams.team_id=A.winner_id FULL JOIN games B ON teams.team_id=B.opponent_id WHERE (A.round='Eighth-Final' AND A.year=2014) OR (B.round='Eighth-Final' AND B.year=2014) ORDER BY name")"
